--[[
   ...
   from Steve Donovan's "luaish.lua" (https://github.com/stevedonovan/luaish.git)
   in order to work with "tide", 2015
--]]

local lpth = require 'path'
lpth.fs = require 'path.fs'
lpth.info = require 'path.info'

local lsh = { tostring = tostring, history='' }
local append = table.insert
local our_completions = {}
...
function lsh.completion_handler(s)
   local cc
...
function lsh.add_shortcuts(shortcuts)
...
function lsh.saveline(s)
   if s ~= lsh.last_line then
      local f = io.open(lsh.history,'a')
      f:write(s)
      if s:sub(-1) ~= '\n' then
         f:write('\n')
      end
...
local function path_candidates(line)
   ...
   local res = {}
   local all = name == ''
   for f, _ in lpth.fs.dir(path) do
...
local function shell_command_handler (line)
   if at(line,1) == '.' then
      line = line:gsub('^%.%s*','')
      ...
      elseif cmd == 'back' then
         back()
      elseif cmd == 'h' then
         return exec(('tail %s |-lf'):format(lsh.history))
